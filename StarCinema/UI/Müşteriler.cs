...
namespace StarCinema.UI
{
    public partial class Müşteriler : Form
    {
        ...
        private void btnMüşteriDüzenle_Click(object sender, EventArgs e)
        {
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = BLogic.MüşteriGüncele(frmMusteri.Musteri);
                if (b)
                {
                    ...
                    row.Cells[4].Value = frmMusteri.Musteri.Email;
        ...
        {
            DataSet ds = BLogic.MüşteriGetir(btnMüşteri.Text);
            if (ds != null)
                dataGridView1.DataSource = ds.Tables[0];
        }
